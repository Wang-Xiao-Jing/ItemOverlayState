...
public class ModNbtUtil {
	...
	public static @NotNull CompoundTag getDataComponentsNbt(ItemStack item, RegistryAccess provider) {
		return ModNbtUtil.getDataComponentsNbt(item.getComponents(), provider);
	}

	public static @NotNull CompoundTag getItemNbt(ItemStack item, HolderLookup.Provider provider) {
		return (CompoundTag) item.saveOptional(provider);
	}

	public static @NotNull CompoundTag getDefaultDataComponentsNbt(ItemStack item, HolderLookup.Provider provider) {
		return getDataComponentsNbt(item.getPrototype(), provider);
	}

	public static @NotNull CompoundTag getExtraDataComponentsNbt(ItemStack item, HolderLookup.Provider provider) {
